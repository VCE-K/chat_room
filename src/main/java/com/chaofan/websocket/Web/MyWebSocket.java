...
public class MyWebSocket {
    ...
    public static HashMap<String,CopyOnWriteArraySet<User>> UserForRoom = new HashMap<String,CopyOnWriteArraySet<User>>(){
        {
            put(HALL, new CopyOnWriteArraySet<>());
        }
    };
    ...
    public void onOpen(Session session) throws IOException {
        ...
        User user = new User(session.getId(),session.getId(),this);
        UserForRoom.get(HALL).add(user);
        RoomForUser.put(session.getId(),HALL);
        Map<String,String> addHallUserResult = new HashMap<>();
        addHallUserResult.put("type","init");
        addHallUserResult.put("msg",session.getId()+"成功加入房间");
        addHallUserResult.put("sendUser","系统消息");
        sendMessagesOther(getUsers(session),gson.toJson(addHallUserResult));
    }
    ...
    public void onClose() {
        ...
        if (users!=null){
            ...
            if (users.size() == 0 && !room.equals(HALL)){
    ...
    public void onMessage(String message, Session session) {
        ...
        Map<String,String> result = new HashMap<>();
        User user = null;
        String shiels = map.containsKey("shiels")? map.get("shiels") :null;
        switch (map.get("type")){
            case "msg" :
            ...
            case "init":
                ...
                if (room != null && nick != null){
                    if (!room.equals(HALL) && RoomForUser.get(session.getId()).equals(HALL)){
                        //先退出大厅
                        CopyOnWriteArraySet<User> users = getUsers(session);
                        user = getUser(session);
                        users.remove(user);
